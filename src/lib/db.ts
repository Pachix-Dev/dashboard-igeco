...
export async function fetchPonenetes(): Promise<Ponentes[]> {        
    try {
        const query = 
        'SELECT s.id, s.uuid, s.name AS speaker_name, COALESCE(s.position_esp, s.position) AS position, s.position_esp, s.position_eng, s.company, s.company_eng, s.bio_esp, s.bio_eng, s.photo, s.impresiones, s.estatus ' + 
