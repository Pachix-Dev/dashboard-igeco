...
export async function PUT(req: Request, { params }: { params: { id: number } }) {
  const { name, position_esp, position_eng, company, company_eng, bio_esp, bio_eng, photo } = await req.json();
  ...
  try {
    ...
    await db.query(
      'UPDATE ponentes SET name = ?, position = ?, position_esp = ?, position_eng = ?, company = ?, company_eng = ?, bio_esp = ?, bio_eng = ?, photo = ? WHERE id = ?',
      [name, position_esp, position_esp, position_eng, company, company_eng, bio_esp, bio_eng, photo, params.id]
    );
