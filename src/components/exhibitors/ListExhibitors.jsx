...
export function ListExhibitors({ exhibitors, onExhibitorUpdated }) {
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredExhibitors.length / itemsPerPage)
  )
            ...
            <tbody className='divide-y divide-white/5'>
              {currentExhibitors.length === 0 ? (
                ...
                currentExhibitors.map((exhibitor, index) => (
                  <tr
                    key={exhibitor.id ?? exhibitor.email ?? index}
                    className='transition hover:bg-white/5'
                  >
                    <td className='px-6 py-4'>
                      <div className='font-semibold text-white'>
                        {exhibitor.name || t('table.noData')}
                      </div>
                      <p className='text-xs text-slate-400'>
                        {exhibitor.lastname || t('table.noData')}
                      </p>
                    </td>
                    <td className='px-6 py-4 text-sm text-slate-200'>
                      {exhibitor.email || t('table.noData')}
                    </td>
                    <td className='px-6 py-4 text-sm text-slate-200'>
                      {exhibitor.position || t('table.noData')}
                    </td>
                    <td className='px-6 py-4'>
