...
export function QrPrinter({ exhibitor }) {
  ...
  const handlePrint = async () => {
    const updatePrintUser = await fetch('/api/exhibitors/printer', {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id: exhibitor.id }),
    })
    ...
    printWindow.document.write(`
      <html>
        ...
        <body>
          ...
          <h1>${exhibitor.name} ${exhibitor.lastname}</h1> 
          <p>${exhibitor.position} <br />${exhibitor.company}</p>          
          <script>
            ...
            <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
              <div className='bg-white p-7 rounded-lg shadow-lg text-center text-black'>
                <div id='qr-code' className='flex justify-center'>
                  <QRCode value={exhibitor?.uuid} size={110} />
                </div>
