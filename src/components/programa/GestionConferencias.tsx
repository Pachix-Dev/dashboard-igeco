...
export function GestionConferencias({
  escenarios,
  dias,
  conferencias,
  onUpdate,
  ...
  const loadConferenciaCompleta = async (id: number) => {
    try {
      ...
      if (conf) {
        ...
        setLogoFile(null);
        setLogoPreview(getLogoUrl(conf.company_logo));
        setIsModalOpen(true);
      }
    } catch (error) {
  ...
  const onSubmit = async (data: ConferenciaForm) => {
    setIsLoading(true);
    try {
      let companyLogo = data.company_logo || "";
      ...
      if (logoFile) {
        ...
        if (!uploadResponse.ok) {
          ...
          notify(uploadError?.error || t("toast.uploadError"), "error");
          return;
        }

        const uploadData = await uploadResponse.json();
        companyLogo = uploadData.path || "";
      }

      const payload = { ...data, company_logo: companyLogo };

      if (editingConferencia) {
        await updateConferencia(editingConferencia.id, {
          active: editingConferencia.active,
          ...payload,
        } as any);
      } else {
        await addConferencia(payload as any);
      }
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between overflow-x-auto">
        ...
        <div className="grid md:flex gap-3">
          ...
          <select
            value={filterDia || ""}
            onChange={(e) =>
              setFilterDia(e.target.value ? Number(e.target.value) : null)
            }
            className="rounded-xl border border-white/10 bg-slate-900/60 px-4 py-2.5 text-sm text-white transition focus:border-blue-400/60 focus:outline-none"
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                ...
                <div className="">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-slate-900/60 px-4 py-2.5 text-sm text-white file:mr-3 file:rounded-lg file:border-0 file:bg-blue-500/20 file:px-3 file:py-1.5 file:text-xs file:font-semibold file:text-blue-200 transition focus:border-blue-400/60 focus:outline-none"
                    />
                    <input type="hidden" {...register("company_logo")} />
                </div>
                 <div className="backdrop-blur-sm rounded-lg border border-white/10 bg-white/5 p-2 flex items-center justify-center">
                   {logoPreview && (
                    
                      <div className="relative h-20 w-40 overflow-hidden rounded-lg border border-white/10 bg-white/5">
                        <Image
                          src={logoPreview}
                          alt="Preview logo"
                          fill
                  ...
                  {loadingPonentes ? (
                    ...
                    <div className="space-y-3">
                      {fields.map((field, index) => (
                        <div
                          key={field.id}
                          className="grid md:flex gap-3 rounded-lg border border-white/10 bg-slate-900/50 p-3"
                        >
                          <div className="flex-1 space-y-3">
                            <select
                              {...register(
                                `ponentes.${index}.ponente_id` as const,
                                {
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              ...
                              >
                                <option value="speaker">
                                  {t("form.ponentes.roles.speaker")}
                                </option>
                                <option value="moderator">
                                  {t("form.ponentes.roles.moderator")}
                                </option>
                                <option value="panelist">
                                  {t("form.ponentes.roles.panelist")}
                                </option>
                                <option value="guest">
                                  {t("form.ponentes.roles.guest")}
                                </option>
                              </select>
                              ...
                              <input
                                type="number"
                                {...register(
                                  `ponentes.${index}.order_index` as const,
                                  { valueAsNumber: true },
                                )}
                                className="rounded-lg border border-white/10 bg-slate-900/60 px-3 py-2 text-sm text-white transition focus:border-blue-400/60 focus:outline-none"
