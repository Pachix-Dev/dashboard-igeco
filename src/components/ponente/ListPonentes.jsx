...
export function ListPonentes({ponente}) {
  ...
  const totalPages = Math.max(1, Math.ceil(filteredPonentes.length / itemsPerPage));
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-200 shadow-lg shadow-blue-500/10">
            ...
            <div>
              ...
              <p className="font-semibold text-white">{t('search.found', {count: filteredPonentes.length})}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/50 shadow-inner shadow-blue-500/5">
          ...
          <table className="min-w-full divide-y divide-white/5">
            ...
            <tbody className="divide-y divide-white/5">
              {currentPonentes.length === 0 ? (
                <tr>
                  <td colSpan={role === 'admin' ? 5 : 4} className="px-6 py-8 text-center text-sm text-slate-400">
                    {t('empty')}
                  </td>
                </tr>
              ) : (
                currentPonentes.map((item, index) => (
                  <tr key={item.uuid ?? item.email ?? index} className="transition hover:bg-white/5">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className="h-12 w-12 overflow-hidden rounded-full border border-white/10 bg-white/5">
                          <img
                            src={`/ponentes/${item.uuid}.jpg`}
                            className="h-full w-full object-cover"
                            alt={item.speaker_name}
                          />
                        </div>
                        <div>
                          <div className="font-semibold text-white">{item.speaker_name || t('table.noData')}</div>
                          <p className="text-xs text-slate-400">{item.position || t('table.noData')}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-sm text-slate-200">{item.email || t('table.noData')}</td>
                    {role === 'admin' && (
                      <td className="px-6 py-4 text-sm text-slate-200">{item.impresiones ?? 0}</td>
                    )}
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap justify-end gap-2">
                        <QrPrinterPonente ponente={item} />
                        <EditPonentes ponente={item} />
                      </div>
                    </td>
                  </tr>
                ))
        ...
        <div className="flex flex-col gap-3 border-t border-white/10 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-sm text-slate-400">{t('pagination.page', {current: currentPage, total: totalPages})}</div>
